...
Atomic density distributions are stored in different directories based on radius and density calculation approaches. Atom count and atomic weight approaches are scaled separately to histograms with same bins and bin limits. We procceed to scatter-plots visualization and analysis with only one computational approach at a time and with all the different radius.
...
write.table(breaks, file="breaks")
...
#setwd("~/project/ptixiaki/testing/")
...
Now scale distributions in 6A radius from cluster 4 and 6 with atom count
...
cluster3_df = read.csv("cluster_3_df")
...
cluster3_ids = cluster3_df$id
cluster3_files <- paste0(cluster3_ids, ".pdb.water")
...
#file.copy(file.path("./solvate_files/", cluster3_files), "cluster_3_files")
...
Raw data for analysis is ready. We can now create a scatter plot to see how data is distributed.
...
postscript("scatter_final_7A.eps", width=5, height=5, horizontal=FALSE, onefile=FALSE, paper="special")
g
dev.off()
...
Plot the mean value of the raw data scatter plot with 2 outlier distributions
...
g = ggplot(melt_raw_data)+
    ...
    geom_line( aes(x=as.numeric(variable), y=mean), color="black", size=0.5 )+
...
Identification of outliers by summing each row from the distance matrix
...
# plot
sums = rowSums(distance_matrix, dims=1)
means <- apply(distance_matrix, 1, mean)
sds <- apply(distance_matrix, 1, sd)
medians <- apply(distance_matrix, 1, median)
maxs <- apply(distance_matrix, 1, max)

data = data.frame(sums, means, sds, medians, maxs)
write.table(data, file = "rowsums_df_7A")

df_5A = read.table("5A/rowsums_df_5A")
df_6A = read.table("6A/rowsums_df_6A")
df_7A = read.table("7A/rowsums_df_7A")

df_5A$radius = "5"
df_6A$radius = "6"
df_7A$radius = "7"

df = rbind(df_5A, df_6A, df_7A)

my_comparisons = list( c("5", "6"), c("6","7"), c("5", "7") )

library(ggplot2)
library(ggpubr)

ggplot(df, aes(x=as.numeric(means), y=as.factor(radius), 
                      color=as.factor(radius)))+
  
  geom_jitter()+
  geom_violin(trim=FALSE)+
  geom_boxplot(width = 0.1)+
  scale_y_discrete(limits=c("5", "6", "7"))+
  labs(x="Mean of distances", 
       y = "Radius", color="Radius")+
...
g = ggplot( pca3d, aes(x= PC1 , y= PC2) )+
  ...
  guides(color = FALSE)+
  geom_text(aes(label = label), color = "black", size = 3, vjust = -1, hjust = +0.5)+
  theme(axis.text.x=element_text(size=14),axis.title.x=element_text(size=16))+
  theme(axis.text.y=element_text(size=14),axis.title.y=element_text(size=16))+
...
#plot3d(pca3d[,1:3], 
#       size=5)
#rgl.postscript("my_3d_plot.eps", fmt = "eps")
...
write.table( hdbscan_df , file="./hdbscan_clusters_df6")
...
cluster8 = which( hierarchical_df$clusters == 8)
cluster11 = which( hierarchical_df$clusters == 11)
cluster3 = which( hierarchical_df$clusters == 3)
...
cluster8_df = data.frame("id" = PDBs[cluster8], "cluster" = 8)
write.csv(cluster8_df, "cluster_8_df", row.names = FALSE)

cluster11_df = data.frame("id" = PDBs[cluster11], "cluster" = 11)
write.csv(cluster11_df, "cluster_11_df", row.names = FALSE)

cluster3_df = data.frame("id" = PDBs[cluster3], "cluster" = 3)
write.csv(cluster3_df, "cluster_3_df", row.names = FALSE)


cluster_df = rbind(cluster4_df, cluster6_df, cluster8_df, cluster11_df)
write.csv(cluster_df, "cluster_4_6_8_11_df", row.names = FALSE)
...
directory = '/home/touliopoulos/project/ptixiaki/filter_pdb_format_obabel/6_MUTATE_FORMATS'
        ...
        for index, row in df.iterrows():
            ...
            if row["id"] in unique_pdbs:
                ...
                with open(file_path, "r" ) as file:
                    try:
                        for line in file:
                            if line[17:20] in list(aminoacid_dictionary.keys()) and  line[77] != "H":
                                three_letter = line[17:20]
                                one_letter = aminoacid_dictionary[three_letter]
                                one_letter_list.append(one_letter)
                    
                                numeric_id = line[22:27]
                                numeric_id = numeric_id.replace(" ", "")
                                numeric_id_list.append(numeric_id)
                    except:
                        print("error in",pdb_id)
    
                    for numerid_id in range(len(numeric_id_list)-1):
                        if numeric_id_list[numerid_id] != numeric_id_list[numerid_id+1]:
                            fasta_sequence = fasta_sequence + one_letter_list[numerid_id]
                    fasta_sequence = fasta_sequence + one_letter_list[numerid_id+1]
...
columns_to_sum <- grep("COILEDCOIL", names(df), value = TRUE)
